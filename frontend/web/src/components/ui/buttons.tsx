import { type ComponentProps } from "react";
import { tv, type VariantProps } from "tailwind-variants";
import {Slot } from "@radix-ui/react-slot";

const buttonVariants = tv({
    base: "text-zinc-400 rounded-lg hover:text-zinc-100 hover:bg-zinc-800 aria-disabled:opacity-50 aria-disabled:pointer-events-none transition-colors flex-shrink-0",

    variants: {
        size: {
            default: "m-200 px-3 sm:px-3 py-2 text-sm sm:text-sm",
            icon: "p-2 sm:p-2",
            "icon-sm": "p-1 sm:p-1",
        },
    },


    defaultVariants: {
        size: "default",
    },

});

type ButtonProps = ComponentProps<"button"> &
  VariantProps<typeof buttonVariants> & {
    asChild?: boolean;
  };

export function Button({ size, className, asChild, ...props }: ButtonProps) {
  const Component = asChild ? Slot : "button";

  return (
    <Component className={buttonVariants({ size, className })} {...props} />
  );
}
